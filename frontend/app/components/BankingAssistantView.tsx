...
import { BankIcon, UserIcon, SendIcon } from './Icons';
...
export default function BankingAssistantView() {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Pointing to the banking endpoint as requested (placeholder if backend route doesn't exist yet)
      const response = await fetch(`${apiUrl}/ask-banking`, {
        method: "POST",
      ...
      const aiMessage: Message = {
        ...
        role: "ai",
        content: data.answer || "This is a placeholder response. The banking backend route might not be fully implemented yet.",
      };
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        role: "ai",
        content: "Sorry, I encountered an error connecting to the banking database. (Ensure the /ask-banking endpoint is running).",
      };
          ...
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center animate-fade-in-up">
              <div className="w-24 h-24 rounded-2xl bg-navy-800 border border-navy-700 flex items-center justify-center shadow-2xl mb-8 relative overflow-hidden group">
                ...
                <BankIcon className="w-12 h-12 text-gold-400 relative z-10" />
              </div>
              <h2 className="text-3xl font-bold text-slate-100 mb-4 tracking-tight">Banking Assistant</h2>
              <p className="text-navy-200 max-w-lg mb-8 leading-relaxed text-lg">
                Get help with banking, RBI guidelines, financial schemes for rural India, PMJDY and more.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-2xl text-left">
                {[
                  "What are the benefits of PMJDY?",
                  "Explain RBI guidelines on KYC.",
                  "What financial schemes exist for farmers?",
                  "How does the direct benefit transfer work?"
                ].map((preset, i) => (
                  <button
                    key={i}
          ...
          {isLoading && (
            <div className="flex items-start gap-4 animate-fade-in">
              ...
              <div className="px-6 py-5 bg-navy-900/80 border border-navy-700 rounded-2xl rounded-tl-sm shadow-md backdrop-blur-sm flex items-center gap-3">
                ...
                <span className="text-sm font-medium text-navy-200">Analyzing banking databases...</span>
              </div>
            </div>
          )}
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            <textarea
              ...
              placeholder="Ask a banking related question... (Press Enter to send)"
          ...
          <div className="flex justify-between items-center mt-3 px-2 text-[11px] font-medium text-navy-400">
            <p>AI may produce inaccurate information. Double-check facts.</p>
